...
window.debouncedRenderUsers = window.debounce(() => {
  window.renderWithFocusRestore(() => window.renderUsers(true));
}, 250);
  ...
  container.innerHTML = `
    ...
    <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-6 shadow-sm flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
      <div class="flex items-center gap-4">
        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#1A3A52] to-[#1E293B] text-amber-400 flex items-center justify-center text-3xl font-extrabold shadow-lg"><i class="bx bx-group"></i></div>
        <div>
          ...
          <h2 class="text-xl font-bold text-[#1A3A52] mt-1">User & Access Control Directory</h2>
          <p class="text-xs text-[#6B7280]">Manage staff terminal accounts, approve or decline registration requests, and configure access roles.</p>
        </div>
      </div>

      <div class="flex items-center gap-2 w-full lg:w-auto">
        <button onclick="openAddUserModal()" class="bg-gradient-to-r from-[#1A3A52] to-[#D4A574] hover:from-[#D4A574] hover:to-[#B45309] text-white border-none rounded-xl px-5 py-3 text-xs font-extrabold cursor-pointer flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20 w-full lg:w-auto transition-all active:scale-95">
          <span><i class="bx bx-plus"></i></span> Add New User
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-5 shadow-xs">
        <div class="text-xs font-bold uppercase tracking-wider text-[#6B7280] mb-1">Total System Users</div>
        <div class="text-2xl font-extrabold text-[#1A3A52]">${totalCount}</div>
        <div class="text-[0.7rem] text-[#6B7280] mt-1">Registered staff terminals</div>
      </div>
      <div class="bg-[#FFFFFF] border ${pendingCount > 0 ? 'border-amber-400 bg-amber-50/20' : 'border-black/[0.1]'} rounded-2xl p-5 shadow-xs">
        ...
        <div class="text-[0.7rem] text-[#6B7280] mt-1">Awaiting admin review</div>
      </div>
      <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-5 shadow-xs">
        <div class="text-xs font-bold uppercase tracking-wider text-[#6B7280] mb-1">Approved & Active</div>
        <div class="text-2xl font-extrabold text-emerald-600">${approvedCount}</div>
        <div class="text-[0.7rem] text-[#6B7280] mt-1">Granted full terminal access</div>
      </div>
      <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-5 shadow-xs">
        <div class="text-xs font-bold uppercase tracking-wider text-[#6B7280] mb-1">Declined Requests</div>
        <div class="text-2xl font-extrabold text-rose-600">${declinedCount}</div>
        <div class="text-[0.7rem] text-[#6B7280] mt-1">Restricted from data access</div>
      </div>
    </div>
    ...
    <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-4 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
      <div class="relative w-full sm:w-80">
        ...
        <input type="text" id="userSearchInput" value="${searchInput}" placeholder="Search name, username, role..." onkeyup="debouncedRenderUsers()" class="w-full bg-[#FFFFFF] border border-black/[0.1] text-[#1A3A52] rounded-xl pl-9 pr-4 py-2 text-xs font-medium focus:outline-none focus:border-[#1A3A52]">
      </div>

      <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
        <select id="userStatusFilter" onchange="renderUsers()" class="bg-[#FFFFFF] border border-black/[0.1] text-xs font-bold text-[#1A3A52] rounded-xl px-3 py-2 focus:outline-none focus:border-[#1A3A52]">
          <option value="ALL" ${statusFilter === 'ALL' ? 'selected' : ''}>All Statuses</option>
          <option value="PENDING_APPROVAL" ${statusFilter === 'PENDING_APPROVAL' ? 'selected' : ''}>⏳ Pending Approval</option>
          <option value="APPROVED" ${statusFilter === 'APPROVED' ? 'selected' : ''}><i class="bx bx-check"></i> Approved / Active</option>
          <option value="DECLINED" ${statusFilter === 'DECLINED' ? 'selected' : ''}><i class="bx bx-x"></i> Declined / Blocked</option>
        </select>

        <select id="userRoleFilter" onchange="renderUsers()" class="bg-[#FFFFFF] border border-black/[0.1] text-xs font-bold text-[#1A3A52] rounded-xl px-3 py-2 focus:outline-none focus:border-[#1A3A52]">
    ...
    <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-5 shadow-sm">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-base font-bold text-[#1A3A52]">Registered Users (${filteredUsers.length})</h3>
        ...
        <table class="data-table w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-black/[0.1]">
            ${filteredUsers.length > 0 ? filteredUsers.map(u => {
              ...
              let statusBadge = '<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-300"><i class="bx bx-check"></i> Active / Approved</span>';
              if (isPending) {
                ...
                statusBadge = '<span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-300"><i class="bx bx-x"></i> Declined / Blocked</span>';
              }
                ...
                <tr class="${isPending ? 'bg-amber-50/30' : (isDeclined ? 'bg-rose-50/30 opacity-75' : '')}">
                  ...
                  <td class="py-3 px-4 text-right whitespace-nowrap">
                    <div class="flex items-center justify-end gap-1.5">
                      ${isPending ? `
                        <button onclick="setUserApprovalStatus('${u.id}', 'APPROVED')" title="Approve Account" class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 cursor-pointer transition-all active:scale-95"><i class="bx bx-check"></i> Approve</button>
                        <button onclick="setUserApprovalStatus('${u.id}', 'DECLINED')" title="Decline Account" class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 cursor-pointer transition-all active:scale-95"><i class="bx bx-x"></i> Decline</button>
                      ` : (isDeclined ? `
                        <button onclick="setUserApprovalStatus('${u.id}', 'APPROVED')" title="Re-approve Account" class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 cursor-pointer transition-all active:scale-95"><i class="bx bx-check"></i> Approve</button>
                      ` : `
                      ...
                      <button onclick="openResetPasswordModal('${u.id}')" title="Reset Password" class="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-bold text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 cursor-pointer transition-all active:scale-95"><i class="bx bx-key"></i> Reset</button>

                      ${u.username !== 'admin' ? `
                        <button onclick="deleteUserAccount('${u.id}')" title="Delete Account" class="inline-flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs font-bold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 cursor-pointer transition-all active:scale-95"><i class="bx bx-trash"></i></button>
                      ` : ''}
                    </div>
                  </td>
                </tr>
...
window.deleteUserAccount = function(userId) {
  ...
  window.showConfirmModal({
    title: '<i class="bx bx-group"></i> Delete User Account',
...
window.renderSecurityAuditLog = function(containerId = 'auditLogContent') {
  ...
  const severityIcons = { INFO: 'ℹ️', WARNING: "<i class=\'bx bx-error\'></i>", CRITICAL: '🚨' };

  container.innerHTML = `
    <div class="bg-[#FFFFFF] border border-black/[0.1] rounded-2xl p-5 shadow-sm">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
        <h3 class="text-base font-bold text-[#1A3A52] flex items-center gap-2">
          <span class="text-xl"><i class="bx bx-lock-alt"></i></span> Security & Access Event Log
        ...
        <div class="flex items-center gap-2">
          <select id="auditCategoryFilter" onchange="renderSecurityAuditLog()" class="bg-[#FFFFFF] border border-black/[0.1] text-xs font-bold text-[#1A3A52] rounded-xl px-3 py-2 focus:outline-none focus:border-[#1A3A52]">
            <option value="ALL">All Categories</option>
            <option value="AUTH"><i class="bx bx-key"></i> AUTH</option>
            <option value="ACCESS">🚫 ACCESS</option>
            <option value="USER_MGMT"><i class="bx bx-group"></i> USER_MGMT</option>
            <option value="FINANCIAL">💰 FINANCIAL</option>
            <option value="DATA"><i class="bx bx-box"></i> DATA</option>
          </select>
          <select id="auditSeverityFilter" onchange="renderSecurityAuditLog()" class="bg-[#FFFFFF] border border-black/[0.1] text-xs font-bold text-[#1A3A52] rounded-xl px-3 py-2 focus:outline-none focus:border-[#1A3A52]">
            <option value="ALL">All Severities</option>
            <option value="INFO">ℹ️ INFO</option>
            <option value="WARNING"><i class="bx bx-error"></i> WARNING</option>
            ...
            <div class="flex items-start gap-3 p-3 rounded-xl border ${sev === 'CRITICAL' ? 'bg-rose-50/50 border-rose-200' : (sev === 'WARNING' ? 'bg-amber-50/50 border-amber-100' : 'bg-slate-50 border-slate-100')}">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-3 mt-1">
                  <span class="text-[0.65rem] font-mono text-slate-400"><i class="bx bx-user"></i> @${window.escapeHTML ? window.escapeHTML(l.user || 'system') : (l.user || 'system')}</span>
